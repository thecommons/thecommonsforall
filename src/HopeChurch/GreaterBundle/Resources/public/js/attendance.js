...
function updateOverallAttendance() {
    ...
    d3.json(url,
        function(error, json) {
            ...
            var attendees = (json[date] && json[date].overallAttendeeCount) ?
                json[date].overallAttendeeCount : 0;
...
var updateDataForDate = function() {
    ...
    d3.json(dateUrl,
        function(error, json) {
            ...
            // update the total count of attendees
            $('#marked-cnt').html(json.length);
...
var updateTable = function(role) {
    ...
    var cols = rows.selectAll("td")
        .data(function(row) {
            var name = row.nameLast ? row.nameFirst + " " + row.nameLast :
                row.nameFirst;
            return [name, row.a_latest_format, row.a_cnt];
        })
        .enter()
        .append("td")
...
$('#update-btn').on('click', function(e) {
    ...
    if(attended_ids.length == 0 && not_attended_ids.length == 0) {
        loadingDone();
        return;
    }
    waiting += 1;
